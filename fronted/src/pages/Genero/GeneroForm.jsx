...
import { getGeneroById, createGenero, updateGenero } from '../../services/generoService';
...
export default function GeneroForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err.response?.data?.mensaje || err.response?.data?.message || 'Error al guardar el género';
      setError(msg);
      console.error(err);
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-in">
      <div className="flex items-center gap-4 mb-6">
        <Link to="/generos" className="p-2 hover:bg-slate-200 rounded-full transition-colors text-slate-600">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              required
              className="w-full p-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow"
          ...
          <div>
            ...
            <textarea
              ...
              onChange={handleChange}
              rows="4"
              className="w-full p-2.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow resize-none"
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium flex items-center gap-2 transition-colors disabled:opacity-70"
            >
